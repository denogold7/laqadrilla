# LaQAdrilla — Contexto del proyecto
...
Todo el trabajo y todas las salidas son **en español panameño**. Esto significa:

- **Tuteo, no voseo:** se usa *tú* (`escribe`, `genera`, `revisa`, `dime`), nunca *vos* (`escribí`, `generá`, `revisá`, `decime`).
- **Sin regionalismos rioplatenses:** se prefiere *aquí* a *acá*. («Planilla» sí se usa en Panamá para la hoja de Excel, así que se mantiene.)
- Tono claro, profesional y neutro-panameño; se evita el lunfardo y los modismos locales que no aporten claridad.

---

## Cómo funciona el flujo

1. **`input/`** — Aquí se ponen los insumos: historias de usuario, documentación del producto, contratos de API y observaciones de errores.
...
4. **`output/`** — Aquí se guardan los artefactos generados, ordenados por tipo.
...
1. Escribe las tablas como **Markdown normal** (`| col | col |`), en cualquier sección.
2. **Al terminar de escribir el `.md`, normaliza el archivo una vez:**
...
- Nunca dejes una tabla en Markdown crudo en el entregable: siempre pasa el `.md` por `formatear_tablas.py`.
- **Sin emojis dentro de las celdas** (descuadran el ASCII): usa texto (`Cubierto`, `Sí`, `No`, `Parcial`).
