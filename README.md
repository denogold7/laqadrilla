...
Un repositorio con un equipo de **agentes especializados** en QA. Tú pones un insumo (una historia, una observación de bug, un contrato de API) en la carpeta `input/`, le pides a Claude Code lo que necesitas, y el agente correspondiente genera el artefacto en la carpeta `output/`.

Los **casos de prueba** se generan como planilla **Excel** (`.xlsx`), acompañada de un **informe de cobertura** en Markdown con ambigüedades y preguntas para el PO. El formato del reporte de bug lo defines editando su plantilla. Y si quieres, esos casos se pueden **ejecutar en un navegador real** (con Playwright MCP) para obtener un **reporte de resultados en HTML**.
...
code .
```
Abre la terminal integrada de VS Code (no hace falta una extensión aparte) y ejecuta:
```bash
claude
```
...
- *"Tomá la observación de `input/bugs/` y arma el reporte de bug."*
...
> 🧱 ¿Quieres entender cómo está armado el repo o sumarle un skill / un server MCP a futuro? Mirá [`ARQUITECTURA.md`](ARQUITECTURA.md).
...
2. Agrégale el frontmatter con `name`, `description` y `tools`. La `description` es clave: es lo que usa Claude Code para saber cuándo invocarlo.
...
Licencia **MIT** — © 2026 **QARMY**. Puedes usarlo, modificarlo y compartirlo libremente; se entrega sin garantías.
