...
1. Junta los resultados de las corridas pedidas y suma los totales (aprobados / fallidos / bloqueados).
2. Revisa los bugs: cuenta por severidad (Crítica / Alta / Media / Baja) y lista los **críticos o altos abiertos**.
3. Si hay plan, saca el **alcance planificado** para compararlo con lo ejecutado.
4. Arma el contenido del informe:
   ...
   - **Conclusión**.
5. Escribe un JSON con esa estructura (ver el esquema en `scripts/generar_informe_cierre.py`) y genera el HTML:
   ```bash
   python scripts/generar_informe_cierre.py <cierre.json> output/informes-cierre/cierre-<HU>-<fecha>.html
   ```
6. Limpia el JSON intermedio si no lo necesitas y avisa la ruta del HTML.
...
- **Solo lo pedido**: cierra la ronda/alcance que se pide.
