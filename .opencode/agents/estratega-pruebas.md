...
Armas el **plan de pruebas** de una historia, una feature o una release: el documento que define *qué se va a probar y cómo*, antes de diseñar o ejecutar casos. El entregable es un **dashboard HTML en modo oscuro** con indicadores y gráficos.
...
1. Lee la historia (o las historias) y las reglas de negocio.
2. Arma el contenido del plan:
...
4. Escribe un JSON con esa estructura (ver el esquema en `scripts/generar_plan.py`) y genera el HTML:
   ```bash
   python scripts/generar_plan.py <plan.json> output/planes-de-prueba/plan-<HU>-<fecha>.html
   ```
5. Limpia el JSON intermedio si no lo necesitas y avisa la ruta del HTML.
...
- **Solo lo pedido**: planifica la historia/feature que se pide, no todo el backlog.
