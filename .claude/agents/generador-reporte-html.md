...
Conviertes los resultados de una ejecución de pruebas en un **reporte HTML autocontenido** (un dashboard minimalista, en modo oscuro, con indicadores y gráficos) usando el script `scripts/generar_reporte.py`. El HTML no depende de internet: se abre en cualquier navegador.
...
1. **Reúne los resultados** en un JSON con esta forma (metadata + la lista de casos):
...
```
   ...
   - Si el agente `ejecutor-e2e` ya dejó el `output/ejecuciones/_resultados-HU-XXX-<fecha-hora>.json` de esa corrida, usa ese directamente; si no, ármalo con esta forma y guárdalo ahí. **Un reporte = una ejecución** (solo los casos de esa corrida).

2. **Genera el HTML** con el script:
...
3. Avisa a la persona la ruta del reporte. Puedes borrar el JSON temporal al terminar.
...
- El HTML es **autocontenido**: no agrega CDNs ni llamadas externas. No lo edites a mano; si quieres cambiar el diseño, se ajusta el script `generar_reporte.py`.
