...
Componentes:
  - page(titulo, eyebrow, meta_html, body)  -> documento HTML completo (con marca LaQAdrilla)
...
def page(titulo, eyebrow, meta_html, body):
    return (f'<!doctype html>\n<html lang="es"><head><meta charset="utf-8">'
            ...
            f'<title>{e(titulo)} — LaQAdrilla</title><style>{CSS}</style></head>'
            f'<body><div class="wrap"><header>'
            f'<span class="brand"><span class="dot">🪖</span>'
            f'<span class="eyebrow">LaQAdrilla · {e(eyebrow)}</span></span>'
            f'<h1>{e(titulo)}</h1>{meta_html}</header>{body}'
            f'<footer>Generado por LaQAdrilla</footer></div></body></html>\n')
