...
def build_html(data):
  ...
  <header>
    <span class="brand"><span class="dot">🪖</span><span class="eyebrow">LaQAdrilla · Reporte de ejecución</span></span>
    <h1>{_e(titulo)}</h1>
    {_chips(data)}
  </header>
  ...
  <footer>Generado por LaQAdrilla</footer>
</div>"""
    return (f'<!doctype html>\n<html lang="es"><head><meta charset="utf-8">'
            f'<meta name="viewport" content="width=device-width, initial-scale=1">'
            f'<title>{_e(titulo)} — Reporte LaQAdrilla</title><style>{CSS}</style></head>'
